...
<?php   for($col_count = 0; (($col_count < $this->_config->get('jg_colcat')) && ($index < count($this->rows))); $col_count++):
          $row = $this->rows[$index]; ?>
    <div class="image_block col-md-<?php echo (int) ((12 / $this->_config->get('jg_colcat'))*2); ?>">
		<?php if($this->_config->get('jg_showtotalcatimages') || $row->isnew): ?>
			<div class="qtyImage">
				<?php if($this->_config->get('jg_showtotalcatimages')): ?>
					<i class="icon-photo-camera"></i><?php echo $row->pictures; ?>
				<?php endif;
				echo $row->isnew; ?>
			</div>
		<?php endif;?>
...
<?php     if(in_array($row->access, $this->_user->getAuthorisedViewLevels())): ?>
            <a href="<?php echo $row->link; ?>">
              <div class="category_name"><?php echo $this->escape($row->name); ?></div>
            </a>
