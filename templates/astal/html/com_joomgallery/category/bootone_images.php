...
/* получаем миниатюру категории */
$db = JFactory::getDBO();
$sql = 'SELECT img.imgfilename, img.catid FROM `#__joomgallery` img INNER JOIN `#__joomgallery_catg` cat ON img.id = cat.thumbnail WHERE cat.cid = '.$this->category->cid;
$db->setQuery($sql);
$thumbCategory = $db->loadAssoc();
$sql = 'SELECT catpath FROM `#__joomgallery_catg` WHERE cid = '.$thumbCategory['catid'];
$db->setQuery($sql);
$pathCategory = $db->loadAssoc();
...
<?php if ($thumbCategory) { ?>
	<div class="cat_thumb">
		<img src="/images/joomgallery/originals/<?php echo $pathCategory['catpath']; ?>/<?php echo $thumbCategory['imgfilename']; ?>" alt="">
	</div>
<?php } ?>
